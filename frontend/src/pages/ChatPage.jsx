    ...
    <div className="flex h-[calc(100vh-6rem)] gap-4 animate-fade-in-up">
      ...
      <div className={`${selectedUser ? 'hidden md:flex' : 'flex'} flex-col w-full md:w-80 shrink-0 glass rounded-2xl overflow-hidden`}>
        <div className="p-4 border-b border-slate-100">
          <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2 mb-3"><MessageCircle className="w-5 h-5 text-teal-500" />Messages</h2>
            ...
            filtered.map(u => (
              <button key={u._id} onClick={() => selectUser(u)}
                ...
                <div className="flex-1 text-left min-w-0">
                  <p className={`text-sm font-bold truncate ${selectedUser?._id === u._id ? 'text-primary-700' : 'text-slate-700'}`}>{u.name}</p>
                  <p className="text-[10px] font-medium text-slate-400 uppercase tracking-tight">{isOnline(u._id) ? 'Online' : 'Offline'}</p>
                </div>
              </button>
            ))
          )}
          ...
          <>
            <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 bg-white/50">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">{isOnline(selectedUser._id) ? 'Online' : 'Offline'}{typing ? ' • typing...' : ''}</p>
              </div>
            </div>
